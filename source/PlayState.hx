...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'world-1':
				dialogue = CoolUtil.coolTextFile(Paths.txt('world-1/world-1Dialogue'));
			case 'adventure':
				dialogue = CoolUtil.coolTextFile(Paths.txt('adventure/adventureDialogue'));
			case 'ow':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ow/owDialogue'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'squigglesville':
				{
						defaultCamZoom = 0.9;
						curStage = 'squigglesville';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('sback'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
	
						var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('sfore'));
						stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						stageFront.updateHitbox();
						stageFront.antialiasing = true;
						stageFront.scrollFactor.set(0.9, 0.9);
